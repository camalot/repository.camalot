...
def _build_gh_readme():
	...
	for plugin_info in plugins_info:
		...
		# dir_items = fnmatch.fnmatch(os.listdir(pi_dir), "*.zip")
	...
	markdown = "# %s\n\n|  | Plugin | Version |\n| ----- | ----- | ----- |\n" % repo_name
	for pd in data:
		print "data[%s]" % pd["name"]
		markdown += "| <img src=\"%s/%s/icon.png\" width=\"24\" /> | [%s](%s) | %s |\n" \
		            % (repo_url, pd["name"], pd["name"], pd["url"], pd["version"])
	print "writing readme.md: %s" % readme_file
